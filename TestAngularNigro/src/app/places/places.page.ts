...
import { ItemInterface } from '../shared/interfaces/itemInterface';
...
export class PlacesPage {
  ...
  getPlace() {
    this._placeService.placesObservable$.subscribe((resultList: PlaceDto[]) => {
      this.placeList = resultList.map((element: PlaceDto) => {
        return {
          id: element.id,
          name: element.city + ", " + element.country + " (" + this._placeService.getSeason(element.favoriteSeason) + ")",
        };
      });
    });
  ...
  private _goToPlaceDetail(id:number) {
    ...
    this._PlaceRouter.navigate(['detail',id], {relativeTo:this._activateRoute});
  }

  private _goToPlaceEdit(id:number) {
    console.log("redirecting to Place editing");
    this._PlaceRouter.navigate(['edit',id], {relativeTo:this._activateRoute});
  }
